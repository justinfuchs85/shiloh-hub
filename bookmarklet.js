...
+'<input id="__shn__" value="'+title.replace(/"/g,'&quot;')+'" style="width:100%;box-sizing:border-box;font-size:13px;padding:8px 10px;border:1px solid #ddd;border-radius:8px;outline:none;color:#222;" /></div>'
...
d.getElementById('__shsv__').onclick=async function(){
  ...
  try{
    var resp=await fetch(DB+'/hub/shoppingRooms.json');
    ...
    var put=await fetch(DB+'/hub/shoppingRooms.json',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(rooms)});
